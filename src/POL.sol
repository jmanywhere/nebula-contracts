...
contract POLv2 is IPOL {
    ...
    //               ERC20 Specific
    // ------------------------------------------------
    string public constant name = "Stake LP V2";
    string public constant symbol = "STOKEv2";
    ...
    function addLiquidityFromBase(
        uint256 _base
    ) public onlyWhitelist returns (uint256 _liquidity) {
    ...
    function removeLiquidityToBase(
        uint256 _liquidity,
        uint256 _tax
    ) public onlyWhitelist returns (uint256 _base) {
        uint256 resBase = baseReserve();
        uint256 resToken = tokenReserve();
    ...
    ) external view returns (uint256 liquidity_amount, uint256 token_amount) {
    ...
    ) external view returns (uint256 liquidity_amount, uint256 base_amount) {
    ...
    function setTaxes(
        bool isBuy,
        uint256[7] calldata taxDistribution
    ) external onlyOwner {
